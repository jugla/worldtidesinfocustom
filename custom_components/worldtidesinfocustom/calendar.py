...
def setup_calendar_devices(
    ...
    tides_calendar_device = TidesCalendarDevice(
        hass, name, unique_id, unit_to_display
    )
...
class TidesCalendarData:
    ...
    async def async_get_events(self, hass, entity, start_date, end_date):
        ...
        # the tide info
        tide_info = give_info_from_raw_data(worldtide_data_coordinator.get_data().get("current_data"))
        epoch_frame_min = start_date.timestamp()
        epoch_frame_max = end_date.timestamp()

        extrema_data = tide_info.give_tide_extrema_within_time_frame(epoch_frame_min, epoch_frame_max)

        for index in range(len(extrema_data.get("extrema_epoch"))):
            tide_epoch = datetime.fromtimestamp(extrema_data.get("extrema_epoch")[index])
            tide_height = extrema_data.get("extrema_value")[index]
            tide_type = extrema_data.get("extrema_type")[index]
            event = {
               "uid": entity,
               "summary": self._name + " " +str(tide_type) + " " + str(round(tide_height*self._convert_meter_to_feet,ROUND_HEIGTH)) + self._measurement_unit,
    ...
    async def async_update(self, entity):
        ...
        if next_tide_data.get("high_tide_time_epoch") > next_tide_data.get("low_tide_time_epoch"):
            tide_epoch = datetime.fromtimestamp(next_tide_data.get("low_tide_time_epoch"))
            tide_height = next_tide_data.get("low_tide_height")
            tide_type = "Low"
        else:
            tide_epoch = datetime.fromtimestamp(next_tide_data.get("high_tide_time_epoch"))
