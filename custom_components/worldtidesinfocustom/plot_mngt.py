...
class Plot_Manager:
    ...
    def compute_new_plot(self, data, current_time):
        ...
        height_data = tide_info.give_tide_prediction_within_time_frame(epoch_frame_min,epoch_frame_max)

        height_value = []
        height_time = []
        for height_index in range(len(height_data.get("height_value"))):
            height_current_value = (
                (height_data.get("height_value"))[height_index] * self._convert_meter_to_feet
            )
            height_value.append(height_current_value)

            height_current_time = (height_data.get("height_epoch"))[height_index]
            height_relative_current_time = (height_current_time - current_time) / 60 / 60
        ...
        current_height_value = [current_height_data.get("current_height") * self._convert_meter_to_feet]

        current_height_time_sample = current_height_data.get("current_height_epoch")
        height_relative_current_time_sample = (current_height_time_sample - current_time) / 60 / 60
